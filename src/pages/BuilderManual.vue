                  ...
                  <section class="space-y-4">
                    <header class="flex items-center justify-between">
                      ...
                      <button
                        ...
                        @click="openManualEditor('premier-league-screen-1')"
                      >
                    ...
                    <div class="relative flex-none h-[520px] w-[260px] overflow-hidden rounded-3xl border border-white/20 bg-slate-900/60 shadow-lg shadow-slate-900/40 dark:border-white/10">
                      <img :src="screensById['premier-league-screen-1']?.media?.src" class="h-full w-full object-cover" alt="Screen 1" />
                      
                      <!-- Ombre haut vers bas -->
                      <div class="absolute inset-0 pointer-events-none rounded-3xl" :style="overlayStyleFor('premier-league-screen-1')"></div>

                      <div class="pointer-events-none absolute inset-0 flex flex-col justify-between p-5">
                        <div class="flex justify-start" v-if="screensById['premier-league-screen-1']?.logo?.visible">
                          <div
                            class="h-16 w-16 overflow-hidden rounded-full text-center shadow-lg backdrop-blur"
                            :style="{ background: screensById['premier-league-screen-1']?.logo?.background, color: screensById['premier-league-screen-1']?.logo?.textColor }"
                          >
                            <img
                              v-if="screensById['premier-league-screen-1']?.logo?.imageUrl"
                              :src="screensById['premier-league-screen-1']?.logo?.imageUrl"
                              alt="Logo écran"
                              class="h-full w-full object-contain"
                            />
                            <div v-else class="flex h-full items-center justify-center text-sm font-semibold">{{ screensById['premier-league-screen-1']?.logo?.label }}</div>
                          </div>
                        </div>
                        <h2 class="absolute bottom-5 left-5 right-5 mt-auto max-h-48 overflow-hidden pr-10 text-3xl font-bold leading-tight text-white whitespace-pre-line break-words line-clamp-4">{{ screensById['premier-league-screen-1']?.title }}</h2>
                      </div>
                      <button type="button" class="absolute right-4 top-4 z-20 rounded-full bg-white/80 px-3 py-1 text-xs font-semibold text-gray-700 shadow-sm transition hover:bg-white" @click="openManualEditor('premier-league-screen-1')">
                        Modifier
                      </button>
                    </div>
                    <!-- Screen 2 -->
                    <div class="relative flex-none h-[520px] w-[260px] overflow-hidden rounded-3xl border border-white/20 bg-slate-900/60 shadow-lg shadow-slate-900/40 dark:border-white/10">
                      <img :src="screensById['premier-league-screen-2']?.media?.src" class="h-full w-full object-cover" alt="Screen 2" />
                      <button type="button" class="absolute right-4 top-4 z-20 rounded-full bg-white/80 px-3 py-1 text-xs font-semibold text-gray-700 shadow-sm transition hover:bg-white" @click="openManualEditor('premier-league-screen-2')">
                        Modifier
                      </button>
                    </div>
                    ...
                    <!-- Screen 3 -->
                    <div class="relative flex-none h-[520px] w-[260px] overflow-hidden rounded-3xl border border-white/20 bg-slate-900/60 shadow-lg shadow-slate-900/40 dark:border-white/10">
                      <img :src="screensById['premier-league-screen-3']?.media?.src" class="h-full w-full object-cover" alt="Screen 3" />
                      
                      <!-- Ombre haut vers bas -->
                      <div class="absolute inset-0 pointer-events-none rounded-3xl" :style="overlayStyleFor('premier-league-screen-3')"></div>

                      <div class="pointer-events-none absolute inset-0 flex flex-col justify-between p-5">
                        <h2 class="absolute bottom-5 left-5 right-5 mt-auto max-h-48 overflow-hidden pr-6 text-3xl font-bold leading-tight text-white whitespace-pre-line break-words line-clamp-4">{{ screensById['premier-league-screen-3']?.title }}</h2>
                      </div>
                      <button type="button" class="absolute right-4 top-4 z-20 rounded-full bg-white/80 px-3 py-1 text-xs font-semibold text-gray-700 shadow-sm transition hover:bg-white" @click="openManualEditor('premier-league-screen-3')">
                        Modifier
                      </button>
                    </div>
                    ...
                    <!-- Screen 4 -->
                    <div class="relative flex-none h-[520px] w-[260px] overflow-hidden rounded-3xl border border-white/20 bg-slate-900/60 shadow-lg shadow-slate-900/40 dark:border-white/10">
                      <div class="absolute inset-0" :style="backgroundStyleFor('premier-league-screen-4')"></div>

                      <div class="pointer-events-none absolute inset-0"></div>

                      <div class="pointer-events-none absolute inset-0 z-10 flex flex-col items-center justify-start pt-4">
                        <h2 class="max-h-28 overflow-hidden px-6 text-center text-xl font-bold leading-tight text-white whitespace-pre-line break-words drop-shadow-lg line-clamp-3">{{ screensById['premier-league-screen-4']?.title }}</h2>
                        <p class="mt-3 max-h-28 overflow-hidden px-6 text-center text-xs leading-snug text-white/90 whitespace-pre-line break-words drop-shadow-md line-clamp-3">{{ screensById['premier-league-screen-4']?.subtitle }}</p>
                      </div>
                      ...
                      <button type="button" class="absolute right-4 top-4 z-20 rounded-full bg-white/80 px-3 py-1 text-xs font-semibold text-gray-700 shadow-sm transition hover:bg-white" @click="openManualEditor('premier-league-screen-4')">
                        Modifier
                      </button>
                    </div>
                    ...
                    <!-- Screen 5 -->
                    <div class="relative flex-none h-[520px] w-[260px] overflow-hidden rounded-3xl border border-white/20 bg-slate-900/60 shadow-lg shadow-slate-900/40 dark:border-white/10">
                      <div class="absolute inset-0" :style="backgroundStyleFor('premier-league-screen-5')"></div>

                      <div class="pointer-events-none absolute inset-0"></div>

                      <div class="pointer-events-none absolute inset-0 z-10 flex flex-col items-center justify-start pt-4">
                        <h2 class="max-h-28 overflow-hidden px-6 text-center text-xl font-bold leading-tight text-white whitespace-pre-line break-words drop-shadow-lg line-clamp-3">{{ screensById['premier-league-screen-5']?.title }}</h2>
                        <p class="mt-3 max-h-28 overflow-hidden px-6 text-center text-xs leading-relaxed text-white/90 whitespace-pre-line break-words drop-shadow-md line-clamp-3">{{ screensById['premier-league-screen-5']?.subtitle }}</p>
                      </div>
                      ...
                      <button type="button" class="absolute right-4 top-4 z-20 rounded-full bg-white/80 px-3 py-1 text-xs font-semibold text-gray-700 shadow-sm transition hover:bg-white" @click="openManualEditor('premier-league-screen-5')">
                        Modifier
                      </button>
                    </div>
                  </div>
...
const resolvedScreens = computed(() => {
  return baseScreens.map((screen, index) => {
    ...
    if (mergedLogo?.imageUrl) {
      mergedLogo.background = 'transparent'
    }
...
const screenFourMockup = computed(() => screensById.value['premier-league-screen-4']?.mockup ?? null)
const screenFiveMockup = computed(() => screensById.value['premier-league-screen-5']?.mockup ?? null)
